...
function dataProvider(wordList, dictListLength){
  ...
  for (var i in wordList){ 
    // console.log(i+"::: dictList."+wordList[i].key+" = "+wordList[i].mean)
...
function printOnDiv(wordObject) {
  ...
  searchBtn.addEventListener('click', function(event) {
    ...
    var removeDiv = document.getElementById(wordObject.key);
    removeDiv.remove(removeDiv);
    console.log("remove "+wordObject.key);
  });
};

function removeData(selectKey) {
  //현재 페이지의 단어를 모아놓는 Dictionary에서 해당 단어 제거 로직
  if (wordList.length == dictListLength) {
    for (var i=0; i<wordList.length; i++) {
      // console.log("wordList["+i+"] = "+wordList[i].key);
      
      //dictionary에서 원소 제거하는방법
      // delete thisIsObject[key];
      
    }
  }

  //DB에 저장되어있는 단어를 제거하는 로직
  console.log("wordList to Server");
};
